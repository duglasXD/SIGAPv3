          ...
          $contador=0;
            if($actividad->num_rows() > 0){
                foreach ($actividad->result() as $fila) {
                  ...
                  }elseif($fila->ESTADO_ASOCIACION == "4"){ 
                    echo '<td><span class="label label-primary">Denegado</span></td>';
                  }elseif($fila->ESTADO_ASOCIACION == "5"){ 
                    echo '<td><span class="label label-primary">Cancelado</span></td>';
                  }
                  echo "<td>";
                  $array = array($fila->ID_ASOCIACION,$fila->NUMERO_ASOCIACION,$fila->NOMBRE_ASOCIACION,$fila->SIGLAS_ASOCIACION,$fila->TELEFONO_ASOCIACION,$fila->EMAIL_ASOCIACION,$fila->DIRECCION_ASOCIACION,$fila->INSTITUCION_PERTENECE_ASOCIACION,$fila->ID_MUNICIPIO_ASOCIACION,$fila->HOMBRES_ASOCIACION,$fila->MUJERES_ASOCIACION,$fila->ID_TIPO_ASOCIACION,$fila->ID_SECTOR_ASOCIACION,$fila->ID_CLASE_ASOCIACION,$fila->FOLIO_ASOCIACION,$fila->LIBRO_ASOCIACION,$fila->REG_ASOCIACION,$fila->ARTICULO_ASOCIACION,$fila->FECHA_CONSTITUCION_ASOCIACION,$fila->FECHA_RESOLUCION_FINAL_ASOCIACION,$fila->ESTADO_ASOCIACION,$fila->AFILIACION_ID_ASOCIACION);
                   
                  if(tiene_permiso($segmentos=1,$permiso=4)){
                    array_push($array, "edit");
                    echo generar_boton($array,"cambiar_editar","btn-info","fa fa-wrench","Editar");
                  }
                   if(tiene_permiso($segmentos=1,$permiso=2)){
                     unset($array[endKey($array)]);
                    array_push($array, "add");
                    echo generar_boton($array,"cambiar_editar","btn-danger","fa fa-users","Agregar Afiliados");
                  }
                  if(tiene_permiso($segmentos=1,$permiso=3)){
                    unset($array[endKey($array)]); //eliminar el ultimo elemento de un array
                    array_push($array, "delete");
                    if($fila->ESTADO_ASOCIACION != "0"){
                        echo generar_boton($array,"cambiar_editar","btn-warning","fa fa-chevron-down","Dar de baja");
                    }else{
